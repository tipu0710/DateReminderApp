...
public class EventLoanerAdapter extends RecyclerView.Adapter<EventLoanerAdapter.ViewHolder> {
    List<Person> personList;
    Context context;

    public EventLoanerAdapter(List<Person> personList, Context context) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.txtDate.setText(personList.get(position).getLoanDate());
        holder.txtPhoneNo.setText(personList.get(position).getPhoneNo());
        String time = personList.get(position).getTimeLoner();
        if (time.isEmpty()){
            time = "0:0";
        }
